...
| C13 | ⏭ **Descoped, not fixed** | See dedicated explanation below — this is the one item not implemented. |
...
| C18 | ⏭ Documented, not fixed | Left as designed — the real fix (server-side persisted detector runs referenced by ID) is a bigger design change than fits opportunistically. A code comment now states this explicitly at the `live_event_ids` docstring so it reads as a deliberate deferral, not an oversight. |
