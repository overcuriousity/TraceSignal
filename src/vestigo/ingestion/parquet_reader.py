"""Server-side reader for Vestigo interchange Parquet files.

Converter scripts (``assets/converters/*2vestigo.py``) parse raw evidence
...
:class:`~vestigo.models.event.Event` would derive for the same inputs
(see ``derive_event_id``).
"""
    ...
    def _stamp_batch(
        ...
        for name in ("file_hash", "byte_offset", "content_hash", "source_file"):
            if batch.column(name).null_count:
                raise ValueError(
                    f"Vestigo Parquet file has null values in provenance "
