"""Core event and provenance models for Vestigo.
...
def derive_event_id(
    *,
    ...
    """
    namespace = uuid.uuid5(uuid.NAMESPACE_URL, f"vestigo:{case_id}")
    digest_input = (
        f"{source_id}\n"
        f"{source_identity}\n"
        f"{byte_offset}\n"
...
class Event:
    ...
    def text_for_embedding(
        ...
        ``vestigo.ingestion.pipeline._text_for_embedding`` using a transient
        ...
        """
        from vestigo.ingestion.pipeline import _text_for_embedding
