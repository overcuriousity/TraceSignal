...
from vestigo.db._offsets import (
    bind_offset_params,
    effective_ts_sql,
    offset_raw_bounds,
)
...
from vestigo.db.viz_cache import baseline_cache
...
class EventQueryService:
    ...
    def _build_where(self, query: EventQuery) -> tuple[str, dict[str, Any]]:
        ...
        if query.start is not None or query.end is not None:
            ...
            builder.conditions.append(VESTIGO_NOT_SENTINEL_SQL)
    ...
    def list_fields_by_artifact(
        ...
        *,
        ...
        heuristic→pairs strategy (see :mod:`vestigo.db.field_recommend`).

        When multiple sources are passed the recommendation uses
        :func:`~vestigo.db.field_recommend.recommend_fields_across_sources`
    ...
    def field_value_timeseries(
        ...
        fused_result = self.store.client.query(
            ...
            FROM (
                SELECT ({VESTIGO_NOT_SENTINEL_SQL}) AS ok,
