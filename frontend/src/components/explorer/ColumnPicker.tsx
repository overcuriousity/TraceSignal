    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm">
          ...
          Columns
          {activeCount > 0 && (
            <span className="ml-1 rounded bg-[var(--color-accent-dim)] px-1 text-[10px] font-semibold text-[var(--color-accent)]">
              {activeCount}
            </span>
          )}
        </Button>
      ...
      <PopoverContent className="w-64 p-0" align="end">
        ...
        <div className="max-h-72 overflow-y-auto px-1 py-1">
          {isLoading ? (
            ...
            <>
              {standard.length > 0 && (
                <div>
                  <p className="px-2 pb-1 pt-1.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--color-fg-muted)]">
                    Standard
                  </p>
                  {standard.map((c) => (
                    <ColumnRow
        ...
        <div className="border-t border-[var(--color-border)] p-2">
          <button
            className="flex items-center gap-1.5 text-[10px] text-[var(--color-fg-muted)] hover:text-[var(--color-fg-primary)] transition-base"
